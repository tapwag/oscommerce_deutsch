...
function check_form() {
  ...
  if ( ((keywords == '') || (keywords.length < 1)) && ((dfrom == '') || (dfrom.length < 1)) && ((dto == '') || (dto.length < 1)) && ((pfrom == '') || (pfrom.length < 1)) && ((pto == '') || (pto.length < 1)) ) {
    error_message = error_message + "* <?php echo JS_ERROR_AT_LEAST_ONE_INPUT; ?>\n";
  ...
  if (dfrom.length > 0) {
    if (!IsValidDate(dfrom, '<?php echo DOB_FORMAT_STRING; ?>')) {
      error_message = error_message + "* <?php echo JS_ERROR_INVALID_FROM_DATE; ?>\n";
  ...
  if (dto.length > 0) {
    if (!IsValidDate(dto, '<?php echo DOB_FORMAT_STRING; ?>')) {
      error_message = error_message + "* <?php echo JS_ERROR_INVALID_TO_DATE; ?>\n";
  ...
  if ((dfrom.length > 0) && (IsValidDate(dfrom, '<?php echo DOB_FORMAT_STRING; ?>')) && (dto.length > 0) && (IsValidDate(dto, '<?php echo DOB_FORMAT_STRING; ?>'))) {
    if (!CheckDateRange(document.advanced_search.dfrom, document.advanced_search.dto)) {
      error_message = error_message + "* <?php echo JS_ERROR_TO_DATE_LESS_THAN_FROM_DATE; ?>\n";
      error_field = document.advanced_search.dto;
      error_found = true;
    }
  }
  ...
  if (pfrom.length > 0) {
    ...
    if (isNaN(pfrom_float)) {
      error_message = error_message + "* <?php echo JS_ERROR_PRICE_FROM_MUST_BE_NUM; ?>\n";
  ...
  if (pto.length > 0) {
    ...
    if (isNaN(pto_float)) {
      error_message = error_message + "* <?php echo JS_ERROR_PRICE_TO_MUST_BE_NUM; ?>\n";
  ...
  if ( (pfrom.length > 0) && (pto.length > 0) ) {
    if ( (!isNaN(pfrom_float)) && (!isNaN(pto_float)) && (pto_float < pfrom_float) ) {
      error_message = error_message + "* <?php echo JS_ERROR_PRICE_TO_LESS_THAN_PRICE_FROM; ?>\n";
      error_field = document.advanced_search.pto;
      error_found = true;
    }
  }
