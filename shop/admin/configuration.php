...
*/
  ...
  $cfg_group = tep_db_fetch_array($cfg_group_query);
    ...
    // Start Admin Language Pack
    $str = 'CONFIGURATION_GROUP_' . $cfg_group['configuration_group_id'] . "_TITLE";

    if(defined($str)){
      $cfg_group['configuration_group_title'] = constant($str);
    }  
	// End Admin Language Pack
...
<?php
  $configuration_query = tep_db_query("select configuration_id, configuration_key, configuration_title, configuration_value, use_function from " . TABLE_CONFIGURATION . " where configuration_group_id = '" . (int)$gID . "' order by sort_order");
  while ($configuration = tep_db_fetch_array($configuration_query)) {
 	
 	// Start Admin Language Pack
    
    $str = $configuration['configuration_key'] . "_TITLE";

    if(defined($str)){
    	
      $configuration['configuration_title'] = constant($str);
    }
    ...
    if ((!isset($HTTP_GET_VARS['cID']) || (isset($HTTP_GET_VARS['cID']) && ($HTTP_GET_VARS['cID'] == $configuration['configuration_id']))) && !isset($cInfo) && (substr($action, 0, 3) != 'new')) {
     ...
     // Start Admin Language Pack	
      $str = $cfg_extra['configuration_key'] . "_DESCRIPTION";

      if(defined($str)){
          $cfg_extra['configuration_description'] = constant($str);
      }
      // End Admin Language Pack
