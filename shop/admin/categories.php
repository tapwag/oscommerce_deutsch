...
//osc-support-edition BOF
								 'products_baseprice' => tep_db_prepare_input($HTTP_POST_VARS['products_baseprice']),
								 'products_baseprice_unit' => tep_db_prepare_input($HTTP_POST_VARS['products_baseprice_unit']),
...
//osc-support-edition BOF
            $product_query = tep_db_query("select products_quantity, products_model, products_image, products_price, products_baseprice, products_baseprice_unit, products_date_available, products_weight, products_tax_class_id, manufacturers_id from " . TABLE_PRODUCTS . " where products_id = '" . (int)$products_id . "'");
//osc-support-edition EOF
...
//osc-support-edition BOF
            tep_db_query("insert into " . TABLE_PRODUCTS . " (products_quantity, products_model,products_image, products_price, products_baseprice, products_baseprice_unit,  products_date_added, products_date_available, products_weight, products_status, products_tax_class_id, manufacturers_id) values ('" . tep_db_input($product['products_quantity']) . "', '" . tep_db_input($product['products_model']) . "', '" . tep_db_input($product['products_image']) . "', '" . tep_db_input($product['products_price']) . "', '" . tep_db_input($product['products_baseprice']) . "', '" . tep_db_input($product['products_baseprice_unit']) . "',  now(), " . (empty($product['products_date_available']) ? "null" : "'" . tep_db_input($product['products_date_available']) . "'") . ", '" . tep_db_input($product['products_weight']) . "', '0', '" . (int)$product['products_tax_class_id'] . "', '" . (int)$product['manufacturers_id'] . "')");
//osc-support-edition EOF
...
//osc-support-edition BOF
      $product_query = tep_db_query("select pd.products_name, pd.products_description, pd.products_url, p.products_id, p.products_quantity, p.products_model, p.products_image, p.products_price, p.products_baseprice, p.products_baseprice_unit, p.products_weight, p.products_date_added, p.products_last_modified, date_format(p.products_date_available, '%Y-%m-%d') as products_date_available, p.products_status, p.products_tax_class_id, p.manufacturers_id from " . TABLE_PRODUCTS . " p, " . TABLE_PRODUCTS_DESCRIPTION . " pd where p.products_id = '" . (int)$HTTP_GET_VARS['pID'] . "' and p.products_id = pd.products_id and pd.language_id = '" . (int)$languages_id . "'");
//osc-support-edition EOF
      $product = tep_db_fetch_array($product_query);
...
//osc-support-edition BOF
function updateGrossBaseprice() {
  var taxRate = getTaxRate();
  var grossValue = document.forms["new_product"].products_baseprice.value;

  if (taxRate > 0) {
    grossValue = grossValue * ((taxRate / 100) + 1);
  }

  document.forms["new_product"].products_baseprice_gross.value = doRound(grossValue, 4);
}

function updateNetBaseprice() {
  var taxRate = getTaxRate();
  var netValue = document.forms["new_product"].products_baseprice_gross.value;

  if (taxRate > 0) {
    netValue = netValue / ((taxRate / 100) + 1);
  }

  document.forms["new_product"].products_baseprice.value = doRound(netValue, 4);
}
//osc-support-edition EOF
...
?>
          <tr bgcolor="#ebebff">
            <td class="main"><?php echo TEXT_PRODUCTS_BASEPRICE_NET; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_baseprice', $pInfo->products_baseprice, 'onKeyUp="updateGrossBaseprice()"'); ?></td>
          </tr>
          <tr bgcolor="#ebebff">
            <td class="main"><?php echo TEXT_PRODUCTS_BASEPRICE_GROSS; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_baseprice_gross', $pInfo->products_baseprice, 'OnKeyUp="updateNetBaseprice()"'); ?></td>
          </tr>
          <tr bgcolor="#ebebff">
            <td class="main"><?php echo TEXT_PRODUCTS_BASEPRICE_UNIT; ?></td>
            <td class="main"><?php echo tep_draw_separator('pixel_trans.gif', '24', '15') . '&nbsp;' . tep_draw_input_field('products_baseprice_unit', $pInfo->products_baseprice_unit); ?></td>
          </tr>
<?php 
//osc-support-edition EOF
?>
          ...
          <tr>
            ...
            <td><table border="0" cellspacing="0" cellpadding="0">
              <tr>
                ...
                <td class="main"><?php echo tep_draw_textarea_tinymce('products_description[' . $languages[$i]['id'] . ']', 'soft', '70', '15', (empty($pInfo->products_id) ? '' : tep_get_products_description($pInfo->products_id, $languages[$i]['id']))); ?></td>
              </tr>
            </table></td>
          </tr>
<?php
