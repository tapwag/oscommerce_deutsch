...
?>
          ...
          <tr>
            <td><?php echo '<strong>' . HEADING_SHIPPING_METHOD . '</strong> <a href="' . tep_href_link(FILENAME_CHECKOUT_SHIPPING, '', 'SSL') . '"><span class="orderEdit">(' . TEXT_EDIT . ')</span></a>';
              list($lieferdatum_von,$lieferdatum_bis) = get_lieferzeitraum($order->delivery['country_id']);
              echo '<br><br>'. TEXT_LIEFERUNG .  $lieferdatum_von . '-' . $lieferdatum_bis;
      ?></td>
...
<?php
  ...
  for ($i=0, $n=sizeof($order->products); $i<$n; $i++) {
    echo '          <tr>' . "\n" .
         ...
         '            <td align="left" valign="bottom" width="30"><b>' . $order->products[$i]['qty'] . '&nbsp;x</b></td>' . "\n" .
         '            <td valign="bottom"><b>' . $order->products[$i]['name'] . '</b>';
    ...
    echo '            <td align="right" valign="bottom">' . $currencies->display_price($order->products[$i]['final_price'], $order->products[$i]['tax'], $order->products[$i]['qty']) . '</td>' . "\n" .
         '          </tr>' . "\n";
    echo '          <tr>' .
         '              <td colspan="3" width="100%">
                            <div class="termsContainer">
                                <table border="0" width="100%" cellspacing="0" cellpadding="2"><tr><td valign="top">' . tep_image(DIR_WS_IMAGES . $order->products[$i]['image'], $order->products[$i]['name'], SMALL_IMAGE_WIDTH, SMALL_IMAGE_HEIGHT) . '</td><td valign="top">' . $order->products[$i]['description'] . '</td></tr></table>
                            </div>
                        </td>' .
         '          </tr>';
  }
